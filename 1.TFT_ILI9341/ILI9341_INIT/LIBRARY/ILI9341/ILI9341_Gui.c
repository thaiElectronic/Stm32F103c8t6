...
void ILI9341_Gui_Clear(u16 Color){
  unsigned int i,m;  
	ILI9341_SetWindows(0,0,ili_dev.width-1,ili_dev.height-1);   
	ILI9341_PIN_CS_RESET;
	ILI9341_PIN_DC_SET;
	
	for(i=0;i<ili_dev.height;i++){
    for(m=0;m<ili_dev.width;m++){	
			ILI9341_Send_Data_16Bit(Color);
		}
	}
	ILI9341_PIN_CS_SET;
} 
...
void ILI9341_Gui_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u32 color){  	
	...
	u16 width=ex-sx+1; 		
	u16 height=ey-sy+1;		
	ILI9341_SetWindows(sx,sy,ex,ey);
	for(i=0;i<height;i++){
		for(j=0;j<width;j++){
			ILI9341_Send_Data_16Bit(color);	
		}
...
/*****************************************************************************
 * @name       :void ILI9341_Gui_Direction(u8 direction)
 * @date       :2018-08-09 
...
{ 
	...
	switch(direction){		  
		case 0:						 	 		
			ili_dev.width=ILI9341_W;
			ili_dev.height=ILI9341_H;		
			ILI9341_WriteReg(0x36,(1<<3)|(0<<6)|(0<<7));//BGR==1,MY==0,MX==0,MV==0
		break;
		case 1:
			ili_dev.width=ILI9341_H;
			ili_dev.height=ILI9341_W;
			ILI9341_WriteReg(0x36,(1<<3)|(0<<7)|(1<<6)|(1<<5));//BGR==1,MY==1,MX==0,MV==1
		break;
		case 2:						 	 		
			ili_dev.width=ILI9341_W;
			ili_dev.height=ILI9341_H;	
			ILI9341_WriteReg(0x36,(1<<3)|(1<<6)|(1<<7));//BGR==1,MY==0,MX==0,MV==0
		break;
		case 3:
			ili_dev.width=ILI9341_H;
			ili_dev.height=ILI9341_W;
			ILI9341_WriteReg(0x36,(1<<3)|(1<<7)|(1<<5));//BGR==1,MY==1,MX==0,MV==1
		break;	
		default:break;
	}		
}	
...
/*****************************************************************************
 * @name       :void ILI9341_SetCursor(u16 Xpos, u16 Ypos)
 * @date       :2018-08-09 
...
void DrawPoint(u16 x, u16 y){
	...
	ILI9341_Send_Data_16Bit(POINT_COLOR); 
}

/*******************************************************************
 * @name       :void ILI9341_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
 * @date       :2018-08-09 
...
/*****************************************************************************
 * @name       :void ILI9341_DrawRectangle(u16 x1, u16 y1, u16 x2, u16 y2)
 * @date       :2018-08-09 
...
/*******************************************************************
 * @name       :void ILI9341_Gui_DrawPoint(u16 x,u16 y,u16 color)
 * @date       :2018-08-09 
...
void TFT_Draw_Circle_8(int xc, int yc, int x, int y, u16 c){
	ILI9341_Gui_DrawPoint(xc + x, yc + y, c);
	...
	ILI9341_Gui_DrawPoint(xc + y, yc + x, c);
...
/*****************************************************************************
 * @name       :void ILI9341_Gui_Circle(int xc, int yc,u16 c,int r, int fill)
 * @date       :2018-08-09 
...
/*****************************************************************************
 * @name       :void ILI9341_Gui_Draw_Triangel(u16 x0,u16 y0,u16 x1,u16 y1,u16 x2,u16 y2)
 * @date       :2018-08-09 
...
void ILI9431_Gui_Fill_Triangel(u16 x0, u16 y0, u16 x1, u16 y1, u16 x2, u16 y2){
  ...
  if (y0 > y1) {
    ...
    if(x2 < a){
		...
		ILI9341_Gui_Fill(a,y0,b,y0,POINT_COLOR);
    return;
	}
	dx01 = x1 - x0;
	dy01 = y1 - y0;
    ...
    if(a > b){
		...
		ILI9341_Gui_Fill(a,y,b,y,POINT_COLOR);
	}
	sa = dx12 * (y - y1);
	sb = dx02 * (y - y0);
	for(; y<=y2; y++) {
...
void TFT_Show_6x8_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
  u8 temp;
	...
	ILI9341_SetWindows(x,y,x+8-1,y+8-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display
		...
		temp = asc2_6x8[num][pos];//Call 0806 font, you need to take the model definition
			for(t=0;t<8;t++){                 
				if(temp&0x80){
					ILI9341_Send_Data_16Bit(fc);
				}	
				else {
					ILI9341_Send_Data_16Bit(bc); 
				}
				temp<<=1;
			}		
		}	
	...
	ILI9341_SetWindows(0,0,ili_dev.width-1,ili_dev.height-1);//Restore window to full screen     	   
}
...
void TFT_Show_6x12_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
	...
	u16 colortemp=POINT_COLOR;      		   
	num=num-' ';//Get the offset value
	ILI9341_SetWindows(x,y,x+6-1,y+12-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display	
...
void TFT_Show_8x16_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
	...
	u16 colortemp=POINT_COLOR;      		   
	num=num-' ';//Get the offset value
	ILI9341_SetWindows(x,y,x+8-1,y+16-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display		
...
void TFT_Show_12x24_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
  u16 temp;
	...
	u16 colortemp=POINT_COLOR;      		   
	num=num-' ';//Get the offset value
	ILI9341_SetWindows(x,y,x+12-1,y+24-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display
...
void TFT_Show_16x32_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
	...
	u16 colortemp=POINT_COLOR;      		   
	num=num-' ';//Get the offset value
	ILI9341_SetWindows(x,y,x+16-1,y+32-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display	
...
void TFT_Show_24x48_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode){
	...
	u16 colortemp=POINT_COLOR;      		   
	num=num-' ';//Get the offset value
	ILI9341_SetWindows(x,y,x+24-1,y+48-1);//Set a single text display window
	if(!mode){ //Non-overlay mode: The font has a background color, which will overwrite the original display		
...
void ILI9341_Print(u16 x, u16 y, u16 fc, u16 bc, u8 size, u8 *p, u8 mode){      
	while((*p<='~')&&(*p>=' ')){
		if(size == 8){
...
void ILI9341_Print_Bold(u16 x, u16 y, u16 fc, u16 bc, u8 size, u8 *p, u8 mode){
	while((*p<='~')&&(*p>=' ')){
		if(size == 16){
...
/*****************************************************************************
 * @name       :void ILI9341_Print_Num(u16 x,u16 y,u32 num,u8 len,u8 size)
 * @date       :2018-08-09 
