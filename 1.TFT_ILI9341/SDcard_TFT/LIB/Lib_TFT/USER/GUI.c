...
{
	LCD_Fill(x1,y1,x2,y2,LIGHTGRAY);
}
...
{         
    ...
    {   
        ...
        LCD_ShowChar(x,y,POINT_COLOR,LIGHTGRAY,*p,size,mode);
        x+=size/2;
        p++;
    }  
} 
...
void Gui_bmp16(u16 x,u16 y,u16 a, u16 b, const unsigned char *p) //显示a*b QQ图片
{
  	int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+a-1,y+b);//窗口设置
    for(i=0;i<a*b;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
	...
	LCD_SetWindows(0,0,lcddev.width-1,lcddev.height);//恢复显示窗口为全屏	
	
//	printf("\r\nwidth  =%d\r\n", lcddev.width-1);
//	printf("\r\nheight =%d\r\n", lcddev.height-1);
}

void lcd_Tile(u16 x,u16 y,u16 fc, u16 bc)
{  
//		uint8_t size = 0;
    u8 temp;
    u8 pos;
