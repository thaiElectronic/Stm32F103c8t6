...
int i,k;
unsigned char data1[]="LAB AI - PHOTONICS; Hoc Vien Cong Nghe Buu Chinh Vien Thong; Thuy Son Thai Thuy Thai Binh";
...
int main(void){
	...
	for(i = 0; i < 2; i++){
		...
		res = f_open(&fdst, "IMAGE2.txt", FA_OPEN_ALWAYS | FA_WRITE);
		res = f_lseek(&fdst,i*10);
		if(res != FR_OK){
			//printf("open file error : %d\n\r",res);
		}
		else{
			res = f_write(&fdst,data1 + (i*10) , 10, &bw);/* Ghi nó vào t?p dst */
			if(res == FR_OK){
  ...
  res = f_open(&fsrc, "IMAGE2.txt", FA_OPEN_EXISTING | FA_READ);
