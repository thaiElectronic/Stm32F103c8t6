...
//========================variable==========================//
u16 ColorTab[5]={RED,GREEN,BLUE,YELLOW,MAGENTA};//定义颜色数组
//=====================end of variable======================//
...
//绘制固定栏up
	LCD_Clear(WHITE);
	// mau nen cua khu vuc chi dinh(UP)
	LCD_Fill(0,0,lcddev.width,20,BLUE);// mau nen (0-20px);
	//mau nen cua khu vuc chi dinh(DOWN)
//	LCD_Fill(0,20,0,lcddev.height-20,BLACK);
	LCD_Fill(0,lcddev.height-20,lcddev.width,lcddev.height,BLUE);// mau nen (220-240px)
	POINT_COLOR = WHITE;
	Gui_StrCenter(0,2,WHITE,BLUE,str,16,1);//居中显示
	Gui_StrCenter(0,lcddev.height-18,WHITE,BLUE,"PTIT LAB AI-PHOTONICS",16,1);//居中显示
...
//	Show_Str(10,70,BLUE,YELLOW,"24X24:深圳市中文测试",24,1);
	GUI_DrawFont32(10,100,BLUE,YELLOW,"字体测试",1);
	delay_ms(1200);
}
...
void Pic_test(void)
{
	...
	LCD_Fill(0,20,lcddev.width,lcddev.height-20,WHITE);
	Gui_Drawbmp16(30,30,name);
	Show_Str(30+12,75,BLUE,YELLOW,"QQ",16,1);
	Gui_Drawbmp16(90,30,name);
	Show_Str(90+12,75,BLUE,YELLOW,"QQ",16,1);
	Gui_Drawbmp16(150,30,name);
...
////		Gui_Drawbmp16(30,50,name);
////		delay_ms(1000);delay_ms(1000);
////	}
//	LCD_direction(1);
//	DrawTestPage("TRAN THI NGA");
//	Show_Str(20,30,BLUE,YELLOW,Direction[1],16,1);
//	Gui_Drawbmp16(30,50,name);
//	delay_ms(1000);delay_ms(1000);
...
//	LED_Init();
		...
		while(1)
	{
		...
		if(key==1)	//KEY_RIGHT按下,则执行校准程序
		{
			...
			LCD_ShowString(lcddev.width-24,0,16,"RST",1);//显示清屏区域
			POINT_COLOR=colorTemp;
			LCD_Fill(lcddev.width-52,2,lcddev.width-50+20,18,BACK_COLOR); 
		}
		i++;
		if(i==30)
		{
...
{
    while (*Text) {
        LCD_ShowChar( X, Y, Colour, Background_Colour, *Text++, 16, 1);
        X += 8*Size;
    }
}
