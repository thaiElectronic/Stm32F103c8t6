...
void LCD_ConfigIO(void){
  ...
  GPIO_LCD_InitStruction.GPIO_Pin     = LCD_D4|LCD_D5|LCD_D6|LCD_EN|LCD_D7;
...
void LCD_SendCommand(char command){
  LCD_Send4Bit(command >> 4); //Send lower bita
  LCD_Enable();
  LCD_Send4Bit(command);      //Send upper nibble
  LCD_Enable();
}
/*clear creen*/
void LCD_Clear(void){
...
-*-------------------------------------*/
void LCD_Posi_Char(unsigned char row, unsigned char column, char out_char){
	unsigned char add;
	add = (row==1?0x80:0xC0);
	add += (column - 1);
	LCD_SendCommand(add);
...
-*-------------------------------------*/
void LCD_Posi_Str(unsigned char row, unsigned char column, char* text){
	unsigned char add;
	add = (row==1?0x80:0xC0);
	add += (column - 1);
	LCD_SendCommand(add);
